...
test('getSchema (required with without long enough string)', () => {
  const payload: Section = {
    type: 'text',
    description: 'Please explain your reason for applying',
    id: 'e6d8e732-fdf2-494c-b9d0-1fb333c56620',
  ...
  expect(clientSchema.getSchema(payload).safeParse('abc').error).toBeDefined();
  expect(serverSchema.getSchema(payload).safeParse('abc').error).toBeDefined();
});
